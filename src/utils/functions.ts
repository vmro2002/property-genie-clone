 ...
 */
export const queryParamToNumberArray = (
 ...
 */
export function getFilterDefaultsFromQuery(
  ...
  return {
    minPrice: minPriceQuery
      ? !isNaN(Number(minPriceQuery))
        ? Number(minPriceQuery)
        : undefined
      : undefined,
    maxPrice: maxPriceQuery
      ? !isNaN(Number(maxPriceQuery))
        ? Number(maxPriceQuery)
        : undefined
      : undefined,
    categories:
      categoriesQuery && typeof categoriesQuery === "string"
        ? categoriesQuery
        : undefined,
    types:
      typeof typesQuery === "string"
        ? [typesQuery]
        : Array.isArray(typesQuery)
          ? typesQuery
          : [],
    bedRooms: queryParamToNumberArray(bedRoomsQuery),
    bathRooms: queryParamToNumberArray(bathRoomsQuery),
    furnishings:
      typeof furnishingsQuery === "string"
        ? [furnishingsQuery]
        : Array.isArray(furnishingsQuery)
          ? furnishingsQuery
          : [],
  };
}
...
export async function getListingsData(
  ...
  const res = await fetch(
    `${API_ENDPOINT}/properties-mock?${params.toString()}`,
    {
      method: "POST",
      headers: {
...
export const formatFilterSummary = (
  ...
  primaryText += filter.section
    ? capitalizeFirstLetter(filter.section)
    : "Sale";
